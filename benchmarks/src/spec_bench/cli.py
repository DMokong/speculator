...
def run(ctx, prd, matrix, runs):
    ...
    for run_num in range(config.runs_per_combination):
        ...
        for target in config.targets:
            ...
            # One failed chain (judge timeout, dev server crash, ...) must not
            # abort the remaining targets — record an error row and continue.
            try:
                # Generate spec
                ...
                # Same row shape as the adapter_failed rows above so the
                # report generator treats both failure kinds uniformly.
                click.echo(f"  Target chain failed: {e}")
                all_results.append({
                    "target": target.id,
                    "status": "error",
                    "error": str(e),
