...
>
> **When adding a gate: add the registry row FIRST**, then run `bash tests/test-gate-wiring.sh` and let the test failures enumerate every touchpoint still to update. (Every gate shipped between v2.7.0 and v2.9.x was incompletely wired — this registry + test is the mechanism that prevents recurrence.)
...
7. `skills/sdlc-doctor/SKILL.md` — `--init` config template block (opt-in gates: commented block with threshold)
8. `rubrics/evidence-package.md` — conditional check section + Evidence Output Format entry
9. `skills/sdlc/SKILL.md` — routing table, if the gate gets its own subcommand
10. `tests/test-gate-wiring.sh` — usually no change (registry-driven), but extend Layer A if the gate introduces a new touchpoint class
11. `CHANGELOG.md` / `README.md` — document the gate and its config
