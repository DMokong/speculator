...
## 2.20.0 — Close-time knowledge-base fold-in + visualize sheet (2026-07-11)

`/sdlc close` now keeps the comprehension knowledge base current automatically (claw-r9tc): when `gates.comprehension.mode: asbuilt` and the spec's `gate-2c-asbuilt.yml` shows `result: pass`, the close workflow runs **refresh → fold → verify → viz** and commits the `docs/asbuilt/` changes — after the merge lands (merge strategy) or on the feature branch so the bundle update rides the PR diff (pr strategy). The ordering is load-bearing and pinned by structural tests: refresh always precedes fold (fold refuses citations of concepts that don't exist yet — refresh creates them for spec-new files), `extract.ts` is never run before `refresh.ts` (it would erase the drift baseline), and a fold-in failure is logged without ever rolling back the delivery — the same non-blocking policy as SYSTEM-SPEC compaction. Previously this was a documented manual step ("run the fold when a spec closes until that lands"); SPEC-058's own close demonstrated the gap live — its passing artifact's four enrichment drafts sat unfolded until run by hand.

**New: `asbuilt/src/viz.ts` — the OKF visualize sheet, productized** (claw-efne, from the claudeclaw claw-n5a6 prototype): renders the bundle + graph manifest into a single self-contained `docs/asbuilt/viz.html` — concepts, audit provenance, staleness, and file-level call links — with no external binary (unlike `graphify-check --html`, which needs `graphify` and stays an on-demand advisory view). Deterministic by construction (codepoint-sorted, date from `--date`, never the clock); classification-driven test grouping (claw-wsit); script-breakout escaping. Ships with 7 tests and joins the CLI drift protection (eleven pinned modules now). The close workflow regenerates the sheet whenever the bundle changes, so the committed visualization never lags the knowledge it renders.

Docs: `docs/comprehension-workflow.md` gains a "Keeping everything fresh" section — the one-updater-per-artifact contract (refresh owns the manifest and structure, fold owns audited enrichment, viz owns the sheet), the automatic path, manual refresh cadence commands, partial-run recovery, and the viz-vs-graphify distinction. RELEASE.md records the cache-dependency landmine (a resync that excludes `node_modules` silently degrades Gate 2c to LLM-only — run `bun install` in the cached `asbuilt/`; bit live on the v2.19.0 resync).

## 2.19.0 — As-Built Comprehension Quiz Generator (2026-07-11)

New standalone tool (SPEC-058 port from the claudeclaw reference implementation, claw-klcc — same upstreaming pattern as SPEC-051): the `asbuilt-quiz` skill mines already-audited As-Built comprehension material into a throwaway, self-contained multiple-choice quiz HTML — for reviewer self-checks before approving a change and for codebase onboarding. Two scopes share one pipeline: `--scope=pr-diff` quizzes a diff range's graph slice (any git range — no PR required), `--scope=codebase` quizzes a sample of the bundle's enriched concepts. A blinded agent pair does the authoring: `quiz-generator` cold-reads the scoped material and writes 40-60 graph/concept-cited candidates; `quiz-verifier` adversarially audits each one (single defensible answer, plausible-but-wrong distractors) without ever seeing the generator's reasoning or trap markers. A deterministic zero-LLM mechanical check (`quiz-check.ts`) drops malformed/fabricated-citation candidates with named findings before any sampling — hardened at the LLM-JSON trust boundary so a candidate with missing fields becomes a named drop, never a batch-crashing throw.

**Bank mode (default):** the full verified pool (~50 questions) persists as a local JSON and embeds whole in the rendered HTML, which deals a fresh stratified draw of 12 client-side on every page load — F5 or the "New draw" button re-deals with zero regeneration cost. Fixed-draw mode (`--sample`, seeded and reproducible) is retained for giving a team the identical quiz. The single file renders fully offline (no external references; a `file://` sidecar fetch is CORS-blocked, hence inline embedding), and scoring reveals per-question citations and explanations on submit. Regeneration is a first-class skill path: free re-deal from the persisted bank, or full pipeline re-run for brand-new questions.

Nothing is ever committed to the target repo: quiz output lives in `docs/asbuilt/.quiz/`, ignored via the target's local-only `.git/info/exclude` when its own gitignore doesn't already cover it — repo access alone never reveals an answer key. No gate wiring, no evidence writes, no pipeline effect — same shadow ethos as the comprehension gate it builds on. Ships with 5 CLI modules (`quiz-types/sample/concepts/check/render`), 6 test suites (54 tests including CLI-spawn refusal contracts and blinding pins), drift protection pinning the skill's CLI reference to the modules' `CLI_USAGE` strings, and structural pins on both agent prompts.

Reference implementation validated in claudeclaw via the full seven-gate pipeline: Gate 1 8.5, 2a 8.0, 2 pass (83.98% cov), 2b 6.5 (one self-heal retry adding the refusal-contract suite), 2c asbuilt 8.0 — first full-mechanical 2c run in that repo (415 symbols, 0 citation failures) — Gate 3 pass, Gate 4 33/0/0/0.

## 2.18.0 — Risk-level-bound gate enablement (2026-07-07)

Opt-in gate blocks (2a eval-intent, 2b eval-quality, 2c comprehension) accept a `risk_levels:` allowlist (SPEC-057): the gate runs only when the spec frontmatter's `risk_level` (default `medium`) is in the list — a low-risk one-liner skips three scorer dispatches plus the 2c generator/judge pair while consequential specs keep the full pipeline. One predicate, defined once in `lib/gates.md` "Risk-level binding" and read by every surface (gate-check, sdlc-run + phase references, sdlc-status, evidence-package rubric, verify-evidence.sh); the gate-wiring suite pins the binding across all of them (135 checks, mechanically guarded above the 103 baseline). Semantics: binding refines enablement and never overrides an explicit `enabled: false`; absent/out-of-enum/unreadable risk metadata fails safe — the gate runs and the pipeline warns, naming the problem. Gate 4 records a bound-out gate as `n/a` with a rationale naming both sides of the predicate, never as missing evidence; `sdlc-status` renders it as `SKIPPED (risk)`. `doctor --init` ships a default binding of `risk_levels: [medium, high, critical]` on all three opt-in blocks (prime reference fixture updated in byte-lockstep) and the doctor config lint warns on out-of-enum values, empty lists, and bindings placed on required gates. Existing configs without `risk_levels` keys behave byte-identically to v2.17.1.

Also in this release: the 2a/2b scorer agents now record the dimension weights they used in their evidence scorecards, so `verify-evidence.sh` recomputes **all four** scored gates' overalls — SPEC-057's own Gate 4 run verified with zero recomputation skips (previously every package showed "2 skipped"). New fixture-driven runtime suite `tests/test-risk-binding.sh` (10 cases / 35 assertions) covers the verify-evidence predicate including both weight-recompute directions. Docs: `rubrics/comprehension.md`'s EXPERIMENTAL banner now correctly scopes to legacy mode only (asbuilt mode has its own validated bands) and machine-local pointers were removed; sigma recomputation points at the plugin's own `asbuilt/src/sigma-stats.ts`.

Gated by the full seven-gate pipeline: Gate 1 8.4, 2a 7.3, 2b 6.6 (one self-heal retry), 2c asbuilt 8.1 (LLM-only: the diff touches no extractable-language sources), Gate 3 pass after one review-directed conformance fix, Gate 4 33/0/0/0.
